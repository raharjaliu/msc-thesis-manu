...
\item Given a power kernel function $\Phi$ capable of blowing up the dimension to very high dimensions (see the part about kernel function later on) the kernel might settle for unnecessarily sparse dimensional reducing the generalizability of the problem.
\end{itemize}

A relaxation of this problem is known as $\xi$-SVM, defined as following optimization problem:

subject to $y_i(\mathbf{w} \cdot \mathbf{x}_i) \leq 1 \text{ for } i = 1, \cdots \|\mathbf{S}\|$
