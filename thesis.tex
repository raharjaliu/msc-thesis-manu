...
The so-called Laplacian of Gaussian (LoG) method is used to recognize cell in a field of view, 
...
%TODO improve figure readability
...
Figure \ref{fig:pipeline}a shows how the image is sequentially processed from out-of-focus image to cell trees information.

\section{Single cell tracking in the brightfield channel}

To identify and track single cells from movie start, each out-of-focus brightfield image is processed in the following way (see Figure \ref{fig:pipeline}a for pipeline visualization): 

\begin{itemize}
\item First, a Gaussian blur \ref{zuiderveld1994contrast} with a large radius of 50 pixel is applied to each brightfield image (Fig. S1b) to estimate its individual background and identify e.g. gradients at the edges of the imaged area (Fig. S1c) 
\end{itemize}

. We correct the original image by dividing through this background image (Fig. S1d). Next, we normalize the local contrast of each image in one position \ref{kittler1986minimum} to separate foreground from background. To reduce noise from micro-trench margins in the foreground (Fig. S1e), we calculate the pixel-wise intensity average from all timepoints of one position (Fig. S1f) and subtract this from each image to generate a binary image with mainly cells in the foreground (Fig. S1g).  This creates a mask of the cells. Therein, single cells are recognized using the Laplacian of Gaussian detector in the Fiji TrackMate plugin \ref{tinevez2017trackmate} (Fig. S1h) with a blob diameter of 15 inches and a downsampling factor of 2. The detected cells are concatenated to tracks (Fig S1i) and cellular trees using the Linear Assignment Problem (LAP) tracker\ref{jaqaman2008robust} in TrackMate with a maximal frame-to-frame linking distance of 25 inches, a maximal track segment gap closing distance of 35 inches, a maximal track segment gap closing of 4 frames, and a maximal track segment splitting distance of 25 inches.
...
The image and tracking data is combined and post-processed using Python scripts utilizing OpenCV\ref{bradski2008learning}, NumPy\ref{walt2011numpy}, pandas\ref{mckinney2010data} and Matplotlib \ref{hunter2007matplotlib}, and the Jython and Trackmate \ref{tinevez2017trackmate, pedroni2002jython} plugin in Fiji \ref{schindelin2012fiji}. Our code is available at https://github.com/raharjaliu/MA/tree/master/source.
