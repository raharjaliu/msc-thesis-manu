...
$M$ & A stack of images \\
$M_t$ & An RGB image captured at time t (t-th image) \\
...
2.1.5\subsection{Cell treatments}
...
In statistical learning theory, an SVM is a construct which, given training set $\mathbf{S} \subset \mathbf{D}$ with,
...
finds a hyperplane that separates the input by its class. Initially proposed by Cortes and Vapnik in 1995\cite{cortes1995support}, the original authors proposed to transform the data $\mathbf{S}$ in a higher dimensional representation so that it is separable by the kernel function $\Phi$, a method known nowadays as \textbf{kernel trick}.

%TODO put SVM illustration here and more explanation

Given separability in higher dimensional representation, the support vector $\mathbf{w}$ can then be solved by solving following optimizing problem, 

$$
min_{\mathbf{w}}\{\|\mathbf{w}\|\}
$$

This hard-margin only converges only when the data are linearly separable in mapped space (also known as \textbf{feature space}). This is especially bad since:
...
A relaxation of this problem is known as soft margin SVM or $\xi$-SVM. The problem allows misclassification of some training data. In SVM, misclassification occurs when a data point belonging to a certain class $c_j$ is located in the area defined by the margin $\mathbf{w} x_{c_j} = c_j$ for class $c_j$. The error tolerance $\xi$ is thus defined as the proportion of training data that are wrongly classified $\mathbf{S}_w \in \mathbf{S}$ to the entire training data $\mathbf{S}$,

$$
\xi = \frac{\|\mathbf{S}_w\|}{\|\mathbf{S}\|} 
$$
...
Reducing above term we get then the optimization problem of %TODO derive reduction


$$
min_{\mathbf{w}, \epsilon} \{\frac{1}{2} \mathbf{w}^T \mathbf{w} + \lambda \sum_{i=1}^{\|S\|} \xi_i \}
$$

subject to $y_i(\mathbf{w}^T \Phi(S_i) + \epsilon) \leq 1 - \xi_i$ and $\xi_i \leq 0$.
